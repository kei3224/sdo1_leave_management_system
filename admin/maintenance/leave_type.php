	...
	<div class="card-header">
		...
		<?php if ($_settings->userdata('type') == 1): ?>
			<div class="card-tools">
				<a href="javascript:void(0)" class="btn btn-flat btn-primary" id="create_new"><span
					...
					<tbody>
						...
						while ($row = $qry->fetch_assoc()):
							...
							<tr title="<?php echo $row['description'] ?>">
								...
								<td class='text-right'>
									<?php echo number_format($row['default_credit']) ?>
								</td>
								<td class='text-center'>
									<?php if ($row['status'] == 1): ?>
								...
								<td>
									<?php echo ($row['date_updated'] != null) ? date('Y-m-d H:i', strtotime($row['date_updated'])) : date('Y-m-d H:i', strtotime($row['date_created'])); ?>
								</td>
								<td align="center">
									<button type="button"
